...
static void M0TransferHalfComplete(DMA_HandleTypeDef *DmaHandle);
static void M1TransferHalfComplete(DMA_HandleTypeDef *DmaHandle);
static void M0TransferComplete(DMA_HandleTypeDef *DmaHandle);
static void M1TransferComplete(DMA_HandleTypeDef *DmaHandle);
...
int32_t portA_DMAbuffer_0[DMA_BUFFER_SIZE];		// double buffers for port DMA transfers
int32_t portA_DMAbuffer_1[DMA_BUFFER_SIZE];
int32_t portE_DMAbuffer_0[DMA_BUFFER_SIZE];
int32_t portE_DMAbuffer_1[DMA_BUFFER_SIZE];
int32_t clearDMA = 0;
bool portA_DMAbuffer = false;					// indicates which double buffer to use 0 or 1 (false or true)
bool portE_DMAbuffer = false;
...
#undef PERIOD
#define PERIOD 168-1
// 168 MHz timer -> count to 168 = 1 MHz updates -> step high -> step low -> 500 kHz steps

//Pin *debug1, *debug2, * debug3, * debug4;
//---------------------------------------------------------------------------------------------
...
static void M0TransferHalfComplete(DMA_HandleTypeDef *DmaHandle)
{
	if (DmaHandle->Instance == DMA2_Stream2 ) // TIM8 CH1 for GPIO Port A
	{
		portA_DMAbuffer = true; // switch to buffer_1
		portE_DMAbuffer = true;
...
static void M0TransferComplete(DMA_HandleTypeDef *DmaHandle)
{
	if (DmaHandle->Instance == DMA2_Stream2 ) // TIM8 CH1 for GPIO Port A
	{
		...
		HAL_DMA_Start(&hdma_memtomem_dma2_stream0, (uint32_t)&clearDMA, (uint32_t)portA_DMAbuffer_0, DMA_BUFFER_SIZE);
		//debug2->set(0);
	}
	else if (DmaHandle->Instance == DMA2_Stream3)
	{
		//debug2->set(1);
		HAL_DMA_Abort(&hdma_memtomem_dma2_stream1);
		HAL_DMA_Start(&hdma_memtomem_dma2_stream1, (uint32_t)&clearDMA, (uint32_t)portE_DMAbuffer_0, DMA_BUFFER_SIZE);
		//debug2->set(0);
	}
}
...
static void M1TransferHalfComplete(DMA_HandleTypeDef *DmaHandle)
{
	if (DmaHandle->Instance == DMA2_Stream2 ) // TIM8 CH1 for GPIO Port A
	{
		portA_DMAbuffer = false; // switch to buffer_0
		portE_DMAbuffer = false;
...
static void M1TransferComplete(DMA_HandleTypeDef *DmaHandle)
{
	if (DmaHandle->Instance == DMA2_Stream2) // TIM8 CH1 for GPIO Port A
	{
		//debug4->set(1);
		HAL_DMA_Abort(&hdma_memtomem_dma2_stream0);
		HAL_DMA_Start(&hdma_memtomem_dma2_stream0, (uint32_t)&clearDMA, (uint32_t)portA_DMAbuffer_1, DMA_BUFFER_SIZE);
		//debug4->set(0);
	}
	else if (DmaHandle->Instance == DMA2_Stream3)
	{
		//debug4->set(1);
		HAL_DMA_Abort(&hdma_memtomem_dma2_stream1);
		HAL_DMA_Start(&hdma_memtomem_dma2_stream1, (uint32_t)&clearDMA, (uint32_t)portE_DMAbuffer_1, DMA_BUFFER_SIZE);
		//debug4->set(0);
	}
}
...
int main(void)
{
  ...
  //debug1 = new Pin("PE_9", OUTPUT);
  //debug2 = new Pin("PE_10", OUTPUT);
  //debug3 = new Pin("PE_11", OUTPUT);
  //debug4 = new Pin("PE_12", OUTPUT);
  ...
  // Prepare DMA buffers
  memset(portA_DMAbuffer_0, 0, sizeof(portA_DMAbuffer_0));
  memset(portA_DMAbuffer_1, 0, sizeof(portA_DMAbuffer_1));
  memset(portE_DMAbuffer_0, 0, sizeof(portE_DMAbuffer_0));
  memset(portE_DMAbuffer_1, 0, sizeof(portE_DMAbuffer_1));


  // TIM8 CH1 for GPIO Port A - Axes B, C
  HAL_DMA_RegisterCallback(&hdma_tim8_ch1, HAL_DMA_XFER_CPLT_CB_ID, M0TransferComplete);
  HAL_DMA_RegisterCallback(&hdma_tim8_ch1, HAL_DMA_XFER_M1CPLT_CB_ID, M1TransferComplete);
  HAL_DMA_RegisterCallback(&hdma_tim8_ch1, HAL_DMA_XFER_ERROR_CB_ID, TransferError);
  HAL_DMA_RegisterCallback(&hdma_tim8_ch1, HAL_DMA_XFER_HALFCPLT_CB_ID, M0TransferHalfComplete);
  HAL_DMA_RegisterCallback(&hdma_tim8_ch1, HAL_DMA_XFER_M1HALFCPLT_CB_ID, M1TransferHalfComplete);
  //HAL_DMAEx_MultiBufferStart_IT(&hdma_tim8_ch1, (uint32_t)portA_DMAbuffer_0, (uint32_t)&(GPIOA->BSRR), (uint32_t)portA_DMAbuffer_1, DMA_BUFFER_SIZE);
  HAL_DMAEx_MultiBufferStart(&hdma_tim8_ch1, (uint32_t)portA_DMAbuffer_0, (uint32_t)&(GPIOA->BSRR), (uint32_t)portA_DMAbuffer_1, DMA_BUFFER_SIZE);
  __HAL_TIM_ENABLE_DMA(&htim8, TIM_DMA_CC1);

  // TIM8 CH2 for GPIO Port E - Axes X, Y, Z, A
  HAL_DMA_RegisterCallback(&hdma_tim8_ch2, HAL_DMA_XFER_CPLT_CB_ID, M0TransferComplete);
  HAL_DMA_RegisterCallback(&hdma_tim8_ch2, HAL_DMA_XFER_M1CPLT_CB_ID, M1TransferComplete);
  HAL_DMA_RegisterCallback(&hdma_tim8_ch2, HAL_DMA_XFER_ERROR_CB_ID, TransferError);
  HAL_DMA_RegisterCallback(&hdma_tim8_ch2, HAL_DMA_XFER_HALFCPLT_CB_ID, M0TransferHalfComplete);
  HAL_DMA_RegisterCallback(&hdma_tim8_ch2, HAL_DMA_XFER_M1HALFCPLT_CB_ID, M1TransferHalfComplete);
  HAL_DMAEx_MultiBufferStart_IT(&hdma_tim8_ch2, (uint32_t)portE_DMAbuffer_0, (uint32_t)&(GPIOE->BSRR), (uint32_t)portE_DMAbuffer_1, DMA_BUFFER_SIZE);
  //HAL_DMAEx_MultiBufferStart(&hdma_tim8_ch2, (uint32_t)portE_DMAbuffer_0, (uint32_t)&(GPIOE->BSRR), (uint32_t)portE_DMAbuffer_1, DMA_BUFFER_SIZE);
	  ...
	  switch(currentState){
	          ...
	          case ST_START:
	              ...
	              if (!threadsRunning)
	              {
		              ...
		              if (DMAstepgen)
		              {
		            	  printf("\n\rStarting DMA stepgen\n\r");
		            	  for (iterDMA = vDMAthread.begin(); iterDMA != vDMAthread.end(); ++iterDMA) (*iterDMA)->runModule();
		            	  HAL_TIM_OC_Start(&htim8, TIM_CHANNEL_1);
		            	  //HAL_TIM_OC_Start(&htim8, TIM_CHANNEL_2);
		            	  printf("..... DMA thread started\n\r");
		              }
	              }
